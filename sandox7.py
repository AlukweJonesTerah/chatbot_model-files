...
from tensorflow.keras.layers import Input, Embedding, LSTM, Dropout, Dense, BatchNormalization, Flatten
...
x = Embedding(vocabulary + 1, 20)(i)  # Increased embedding size
x = LSTM(50, return_sequences=True)(x)  # Increased LSTM units
x = BatchNormalization()(x)
x = Dropout(0.5)(x)  # Adjusted dropout rate
x = Flatten()(x)
x = Dense(50, activation="relu")(x)  # Added a dense layer
x = Dropout(0.5)(x)
x = Dense(output_length, activation="softmax")(x)
model = Model(i, x)

# Learning Rate Schedule
lr_schedule = schedules.ExponentialDecay(initial_learning_rate=0.001, decay_steps=100, decay_rate=0.9)
